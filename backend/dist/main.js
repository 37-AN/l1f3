...
async function bootstrap() {
  ...
  app.useGlobalInterceptors(
    new logging_interceptor_1.LoggingInterceptor(loggerService),
  );
  ...
  if (configService.get("NODE_ENV") !== "production") {
    const config = new swagger_1.DocumentBuilder()
      ...
      .setDescription(
        "Personal wealth tracking system for Ethan Barnes - Journey to R1.8M net worth",
      )
      .setVersion("1.0")
  ...
  loggerService.log(
    `🚀 LIF3 Financial Dashboard API started on port ${port}`,
    "Bootstrap",
  );
  loggerService.log(
    `📊 Target: R239,625 → R1,800,000 (13.3% progress)`,
    "Bootstrap",
  );
