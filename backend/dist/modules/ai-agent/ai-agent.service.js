...
let AIAgentService = AIAgentService_1 = class AIAgentService {
    constructor(goalRepository, transactionRepository, businessMetricsRepository, googleDriveService) {
    ...
    async getFinancialContext() {
        ...
        const todayRevenue = todayTransactions.reduce((sum, tx) => sum + Number(tx.amount), 0);
    ...
    async analyzeGoalProgress(goalId) {
        try {
            ...
            const response = await this.anthropic.messages.create({
                model: "claude-3-5-sonnet-20241022",
                max_tokens: 3000,
                system: "You are a financial analysis AI focused on goal achievement and strategic planning.",
                messages: [
                    {
                        role: "user",
    ...
    async generateBusinessStrategy() {
        try {
            ...
            const response = await this.anthropic.messages.create({
                model: "claude-3-5-sonnet-20241022",
                max_tokens: 3000,
                system: "You are a business strategy AI advisor specializing in startup growth and revenue optimization.",
                messages: [
                    {
                        role: "user",
...
Focus on actionable strategies that align with the R1,800,000 net worth goal timeline.`,
    ...
    async monitorGoalProgress() {
        try {
            ...
            const netWorthGoal = await this.goalRepository.findOne({
                where: { type: goal_entity_1.GoalType.NET_WORTH, status: goal_entity_1.GoalStatus.ACTIVE },
            });
            if (netWorthGoal) {
                const progress = (Number(netWorthGoal.currentAmount) /
                    Number(netWorthGoal.targetAmount)) *
                    100;
                const daysRemaining = netWorthGoal.daysRemaining;
                const requiredDailyGrowth = (Number(netWorthGoal.targetAmount) -
                    Number(netWorthGoal.currentAmount)) /
                    daysRemaining;
                if (progress < 50 - (daysRemaining / 730) * 50) {
                    this.logger.warn(`Net worth goal falling behind target. Progress: ${progress.toFixed(1)}%, Required daily growth: R${requiredDailyGrowth.toFixed(0)}`);
                }
            }
            const today = new Date();
            const todayTransactions = await this.transactionRepository.find({
            ...
            if (todayRevenue < 4881 && new Date().getHours() > 18) {
                this.logger.warn(`Daily revenue target not met. Current: R${todayRevenue}, Target: R4,881`);
            }
        }
        catch (error) {
    ...
    async updateNetWorth(newAmount) {
        try {
            ...
            this.logger.log(`Net worth updated: R${previousAmount.toLocaleString()} → R${newAmount.toLocaleString()}`);
    ...
    async logBusinessRevenue(amount, description = "43V3R Revenue") {
        try {
            ...
            this.logger.log(`Business revenue logged: R${amount.toLocaleString()} - ${description}`);
            return true;
        }
        catch (error) {
...
], AIAgentService.prototype, "scheduledDailyBriefing", null);
__decorate([
    (0, schedule_1.Cron)("0 */6 * * *"),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], AIAgentService.prototype, "monitorGoalProgress", null);
exports.AIAgentService = AIAgentService = AIAgentService_1 = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(business_metrics_entity_1.BusinessMetrics)),
