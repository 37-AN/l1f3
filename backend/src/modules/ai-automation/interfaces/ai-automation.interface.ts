...
export interface FinancialGoal {
  ...
  automationRules: string[]; // Rule IDs
  metadata: {
    createdAt: Date;
    lastUpdated: Date;
...
export interface GoalMilestone {
  ...
  achievedDate?: Date;
  reward?: string;
  automationTriggers: string[]; // Rule IDs to trigger when milestone reached
}
...
export interface PredictionResult {
  ...
  confidence: number;
  scenarios: {
    optimistic: { amount: number; date: Date; probability: number };
    realistic: { amount: number; date: Date; probability: number };
    pessimistic: { amount: number; date: Date; probability: number };
  };
...
export interface AIRecommendation {
  ...
  impact: {
    ...
    amountIncrease: number; // Additional amount
    riskReduction: number; // Risk percentage reduction
  };
